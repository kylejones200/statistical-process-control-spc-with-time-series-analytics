...
def generate_process_data(start_date: str='2023-01-01', periods: int=100, freq: str='D', mean: float=50, std: float=2, seed: int=42) -> pd.DataFrame:
...
def calculate_control_limits(df: pd.DataFrame, sigma_multiplier: float=3.0) -> Dict[str, float]:
    ...
    return {'mean': mean, 'std_dev': std_dev, 'ucl': ucl, 'lcl': lcl}
...
def plot_control_chart(df: pd.DataFrame, limits: Dict[str, float], out_of_control: pd.Series, output_path: Path, plot: bool = False):
    ...
    ax.plot(df['Time'], df['Value'], label='Process Data', color='#4A90A4', linewidth=1.2, marker='o', markersize=3)
    ax.axhline(limits['mean'], color='#8B6F9E', linestyle='--', linewidth=1.2, label='Mean')
    ax.axhline(limits['ucl'], color='#D4A574', linestyle='--', linewidth=1.2, label='Upper Control Limit (UCL)')
    ax.axhline(limits['lcl'], color='#D4A574', linestyle='--', linewidth=1.2, label='Lower Control Limit (LCL)')
    if out_of_control.any():
        ax.scatter(df['Time'][out_of_control], df['Value'][out_of_control], color='#D4A574', s=50, label='Out of Control', zorder=5)
    ax.set_xlabel('Time')
    ax.set_ylabel('Value')
